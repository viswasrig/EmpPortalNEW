...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($amount_err) && empty($payee_err) && empty($paymentMethod_err) && empty($header_err)){
        ...
        (EnteredBy, refID, Date, Amount, Payee, PaymentMethod, Header, Description, Comments,RecStatus) 
        VALUES (?,?,?,?,?,?,?,?,?,?)";
 
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("ssssssssss", $param_EnteredBy, $param_referenceID, $param_dateOfExp, $param_amount, $param_payee,
            $param_paymentMethod, $param_header, $param_description, $param_comments, $param_RecStatus);
