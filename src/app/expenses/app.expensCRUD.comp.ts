    ...
    getExpenseObjectById = (id) => {
        this.appService.getExpenseObjectById(id).then((resp) => {
            if (resp['success']) {
                ...
                this.expense.ref = !this.expense.ref ? 'rCompany' : this.expense.ref;
       ...
       const userId = this.user && this.user.ID || '';
        return {
            expenseDate: '', associateName: '', associateID: '', comments: '',
             description: '', ref: 'rCompany', header: '', paymentMethod: undefined,
            deductions: undefined, amount: undefined, payee: '',
            userId: userId, ID: ''
        };
    }
